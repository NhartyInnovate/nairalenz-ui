...
import { Sparkles, Send, Paperclip, Mic, Plus, MessageSquare, Loader2 } from "lucide-react";
import { useState, useRef, useEffect } from "react";
import { useFinanceStore } from "@/lib/store";
import { chatWithCopilot, ChatMessage } from "@/lib/copilot-server";
...
function Copilot() {
  ...
  const [chatSessions, setChatSessions] = useState<ChatHistoryItem[]>([
    {
      ...
      messages: [
        {
          role: "assistant",
          content: `Hi ${profile.fullName.split(" ")[0]} — I'm your NairaLens AI financial copilot. I've analyzed your uploaded bank statements. Ask me anything about your cashflow, budgets, savings rate, or custom financial goals!`,
        },
      ],
    },
    {
      ...
      messages: [
        {
          role: "assistant",
          content:
            "I've reviewed your recurring billing history. You have three active services. Would you like me to analyze them?",
        },
      ],
    },
  ]);
  ...
  const handleSend = async (text: string) => {
    ...
    try {
      ...
      console.error(err);
      toast.error("Failed to connect to the Copilot backend.");
      const errorMsg: ChatMessage = {
        role: "assistant",
        content:
          "Sorry, I encountered an issue compiling the response. Please check your internet connection and try again.",
      };
      setChatSessions((prev) =>
        prev.map((s) =>
          s.id === activeChatId ? { ...s, messages: [...updatedMessages, errorMsg] } : s,
        ),
      );
    } finally {
      setIsTyping(false);
            ...
            {chatSessions.map((c) => (
              <li key={c.id}>
                <button
                  ...
                  className={`flex w-full items-start gap-2 rounded-lg px-2.5 py-2 text-left text-xs transition hover:bg-accent ${activeChatId === c.id ? "bg-accent text-foreground" : "text-muted-foreground"}`}
                >
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-muted-foreground">
                  Analyzing {transactions.length} transactions across active statements
                </p>
              </div>
            </div>
            ...
            {activeChat.messages.map((m, idx) => {
              ...
              return (
                <div key={idx} className={`flex ${isUser ? "justify-end" : "justify-start"} gap-3`}>
                  ...
                  <div
                    className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${isUser ? "bg-primary text-primary-foreground shadow-sm" : "border border-border bg-card text-foreground"}`}
                  >
            ...
            <div className="flex flex-wrap gap-1.5">
              {SUGGESTIONS.map((s) => (
                <button
                  ...
                  className="rounded-full border border-border bg-surface/60 px-3 py-1.5 text-[11px] text-muted-foreground transition hover:border-primary/40 hover:text-foreground"
                >
                  {s}
                </button>
              ))}
